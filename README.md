...
A merchant order-operations analytics project exploring how a platform that connects small merchants to marketplaces and coordinates delivery on their behalf can turn raw order and delivery data into real operational intelligence.

**Why this dataset:** [Olist](https://olist.com) connects small and medium merchants to marketplaces and coordinates fulfillment and delivery for them. Its public dataset — 100k+ real, anonymized Brazilian e-commerce orders from 2016–2018 — gave me a realistic, messy dataset to dig into the kind of merchant-order-delivery questions an operations team actually has to answer.
...
- **SQL** — a 9-table PostgreSQL schema with real foreign keys, and 9 analysis queries using joins, CTEs, window functions, and subqueries ([`sql/`](sql/)).
- **Python** — data cleaning, exploratory analysis, and statistical testing (Pearson correlation, ANOVA) with Pandas, NumPy, Matplotlib, Seaborn, and SciPy, regenerating a full report on every run ([`python/eda_analysis.py`](python/eda_analysis.py)).
- **Excel** — a financial model with live formulas (`TREND()` forecasting, a revenue-at-risk what-if calculator) ([`excel/financial_ops_model.xlsx`](excel/financial_ops_model.xlsx)).
- **Power BI** — a dashboard connected live to PostgreSQL, with DAX measures and KPI, trend, geographic, and seller-leaderboard visuals ([`dashboard/order_ops_dashboard.pbix`](dashboard/order_ops_dashboard.pbix)).
- **Streamlit** — a public, interactive version of the dashboard, deployed live ([`streamlit_app/`](streamlit_app/)).
- **Conversational AI / NLP** — a TF-IDF + Logistic Regression intent classifier trained on real customer-support conversations, with a live chatbot-analytics dashboard page ([`nlp_chatbot_analytics/`](nlp_chatbot_analytics/)).
- **Snowflake** — a star-schema data warehouse built from the same source data via an automated ETL pipeline ([`warehouse_etl/`](warehouse_etl/)).
- **MongoDB** — the same order data remodeled as denormalized documents in a live MongoDB Atlas cluster ([`nosql_mongo/`](nosql_mongo/)).
...
docs/              case_study.md
nlp_chatbot_analytics/  conversational AI / intent classification module (see its own README)
